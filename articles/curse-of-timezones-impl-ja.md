...
ここまで Java の [JSR 310: Date and Time API](https://jcp.org/en/jsr/detail?id=310) を何回か参照してきましたが、この JSR 310 については「単なる日付・時刻を扱うだけなのに複雑すぎる!」という声をよく耳にします。が、ここまで書いてきたように、日付・時刻・タイムゾーンというモノがそもそも超複雑な概念なんだ、という認識が必要です。

JSR 310 はこの「超複雑な日付・時刻・タイムゾーンというモノ」をできるだけ忠実にモデル化していて、実装について検討するのにいいモデルケースになると思います。以下では JSR 310 を参考にしつつも、言語固有の話にはなるべく踏み入らずに、実装に関する一般的な考え方を紹介します。
