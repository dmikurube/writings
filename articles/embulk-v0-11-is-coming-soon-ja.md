...
Embulk v0.11.0 のリリース後、新しい機能などはあまり入れずに v1.0 に移行しようと当初は考えていたのですが、いくつか挙がってきた要望を見ると v1.0 の前に入れたほうがいいかもしれない、という議論もしています。

まだどうするか決まっていませんが、その過程で v1.0 の前に v0.12 や v0.13 に突入することがあるかもしれません。ですが少なくとも、今回の v0.9 → v0.10 → v0.11 のような、大きな非互換をともなう変更は入れないつもりです。
...
* Maven 形式のプラグインに対するサポートの改善 (特にインストール)
* プラグイン用の新しいテスト・フレームワーク
* 新しい Java の正式サポート
* コードのリファクタリングと整理

### もしかしたら v1.0 前にやるかもしれないこと
