...
@[card](https://twitter.com/dmikurube/status/1700368419894522113)

特に Embulk は「データ」をあつかうソフトウェアです。ある場所から別の場所にデータを転送した際、たとえばバグによってデータ形式の変換をひとたび間違えてしまえば、膨大な転送済みデータの中から、あとでその間違いを発見・修正するのは至難の技です。

同様に、たとえば特定のバージョン以降ではある設定の挙動が異なる、ということになれば、

ですので Embulk 本体やプラグインに対してプルリクエストをいただいても、「お、よさそう。マージ〜。ありがとうございます〜」でホイホイと気軽にマージするわけにはいきません。
